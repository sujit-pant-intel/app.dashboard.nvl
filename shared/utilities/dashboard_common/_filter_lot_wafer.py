...
dropdown panel, used by both:
  ...
  • yield-dashboard/yld/yield_analysis.py          (master — defines the visual style)
  • yield-dashboard/sicc_cdyn_upm/sicc_cdyn_upm.py  (CSS, table class, JS)

Lives under shared/utilities/dashboard_common/ since it's reused across
multiple dashboards (not private to yield-dashboard).
...
# ── Shared CSS (yield_analysis.py is the master definition) ──────────────────
FILTER_TABLE_CSS = (
...
# ── Shared dropdown filter JS (yield_analysis.py is the master) ──────────────
...
# }
#
# Both yield_analysis.py (IC.ftDdOpen) and sicc_cdyn_upm (tblFtOpen) call
...
// ── Filter-by-Lot/Wafer complete JS (master: yield_analysis.py) ─────────────
var _ftDdState={};
