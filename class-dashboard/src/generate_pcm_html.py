...
.tab-panel{display:none}.tab-panel.active{display:flex;flex-direction:column;flex:1;min-height:0;overflow:hidden}#main-content{display:flex;flex-direction:row;flex:1;min-height:0;overflow:hidden}#tab-content{display:flex;flex-direction:column;flex:1;min-height:0;overflow:hidden}
...
#panel2{width:400px;min-width:180px;flex-shrink:0;background:#fff;display:flex;flex-direction:column;overflow:hidden;border-right:2px solid #d0d7de;transition:width 0.15s}
#panel2.p2-hidden{width:0!important;min-width:0!important;overflow:hidden;border:none}
...
var _PDLY_GRP_P=(function(){var o={};for(var i=1;i<=PCM_DIST_PANELS.length;i++){o[i]=null;}return o;}());
...
function _buildPdlyPanelBar(pn,allParams,sel,grp,h){
  ...
  /* Group-by checkboxes (global VAR_GBY state) */
  var gby='<span style="width:1px;background:#4a6278;align-self:stretch;margin:0 4px"></span>'
    ...
    +'<label style="cursor:pointer;font-size:12px;color:#ecf0f1"><input type="checkbox" class="vgb-cb" value="none" onchange="toggleGby(\'none\')"'+(VAR_GBY.length===0?' checked':'')+'>  None</label>'
    +'<label style="cursor:pointer;font-size:12px;color:#ecf0f1"><input type="checkbox" class="vgb-cb" value="lot" onchange="toggleGby(\'lot\')"'+(VAR_GBY.indexOf('lot')>=0?' checked':'')+'>  Lot</label>'
    +'<label style="cursor:pointer;font-size:12px;color:#ecf0f1"><input type="checkbox" class="vgb-cb" value="wafer" onchange="toggleGby(\'wafer\')"'+(VAR_GBY.indexOf('wafer')>=0?' checked':'')+'>  Wafer</label>'
    +'<label style="cursor:pointer;font-size:12px;color:#ecf0f1"><input type="checkbox" class="vgb-cb" value="layout" onchange="toggleGby(\'layout\')"'+(VAR_GBY.indexOf('layout')>=0?' checked':'')+'>  Layout</label>'
    +'<label style="cursor:pointer;font-size:12px;color:#ecf0f1"><input type="checkbox" class="vgb-cb" value="material" onchange="toggleGby(\'material\')"'+(VAR_GBY.indexOf('material')>=0?' checked':'')+'>  Material</label>';
  /* Height slider */
...
function _buildPdlyCards(params,ak){
...
function buildPropDelayTab(){
  ...
  for(var _pn=1;_pn<=PCM_DIST_PANELS.length;_pn++){
    ...
    if(_pb&&!_PDLY_P_COLLAPSED[_pn]){
      ...
      var _cards=_params.length?_buildPdlyCards(_params,ak):'<div style="padding:16px;color:#888;font-style:italic;font-size:12px">Parameters not found in dataset.</div>';
      _PDLY_H=_oldH;
  ...
  b5.innerHTML=bar3+pillHtml+'<div style="padding:0 14px 10px">'+_buildPdlyCards(params3,ak)+'</div>';
...
function buildXY2Tab(){
  ...
  if(_XY2_TREND!=='none'){
    ...
    Object.keys(grpTd).forEach(function(gk){
      ...
      p.push('<line x1="'+xp(xlo).toFixed(1)+'" y1="'+Math.max(MT,Math.min(MT+plotH,yp(ty1))).toFixed(1)+'"'
        ...
        +' stroke="'+col+'" stroke-width="2" stroke-dasharray="7,3" opacity="0.75"/>');
    });
  }
  ...
  pts.forEach(function(pt){
    ...
    _dp2[col]+='M'+cx.toFixed(1)+','+cy.toFixed(1)+'m-3,0a3,3,0,1,0,6,0a3,3,0,1,0,-6,0';
  });
  Object.keys(_dp2).forEach(function(col){p.push('<path d="'+_dp2[col]+'" fill="'+col+'" opacity="0.72"/>');});
  ...
  var legHtml=(legParts.length
    ...
    :'');
  cont.innerHTML='<div style="display:flex;flex-direction:column">'+p.join('')+legHtml+'</div>';
...
var _FP_ST=(function(){
  ...
  for(var i=0;i<PCM_XY_PANELS.length;i++){
    ['a','b'].forEach(function(h){
      st['fp'+i+h]={xgrp:'',ygrp:'',x:null,ys:null,logX:false,logY:false,die:true,trend:'ols',xmin:null,xmax:null,ymin:null,ymax:null,h:500,ysrch:''};
    });
  }
  return st;
}());
  ...
  if(st.trend!=='none'){
    ...
    Object.keys(grpTd2).forEach(function(gk){
      ...
      p.push('<line x1="'+xpf(xlo).toFixed(1)+'" y1="'+Math.max(MT,Math.min(MT+plotH,ypf(ty1))).toFixed(1)+'"'
        ...
        +' stroke="'+col2+'" stroke-width="2" stroke-dasharray="7,3" opacity="0.75"/>');
    });
  }
  var _dp3={};
  pts.forEach(function(pt){
    ...
    _dp3[col2]+='M'+cx.toFixed(1)+','+cy.toFixed(1)+'m-3,0a3,3,0,1,0,6,0a3,3,0,1,0,-6,0';
  });
  Object.keys(_dp3).forEach(function(col2){p.push('<path d="'+_dp3[col2]+'" fill="'+col2+'" fill-opacity="0.55" stroke="none"/>');});
...
function fpBuild(pid){
  ...
  var gbyHtml='<span style="width:1px;background:#ccc;align-self:stretch;margin:0 2px"></span>'
    ...
    +'<label style="font-size:11px;cursor:pointer;display:flex;align-items:center;gap:2px"><input type="checkbox" class="vgb-cb" value="none" '+(VAR_GBY.length===0?'checked':'')+' onchange="toggleGby(\'none\')"> None</label>'
    +'<label style="font-size:11px;cursor:pointer;display:flex;align-items:center;gap:2px"><input type="checkbox" class="vgb-cb" value="lot" '+(VAR_GBY.indexOf('lot')>=0?'checked':'')+' onchange="toggleGby(\'lot\')"> Lot</label>'
    +'<label style="font-size:11px;cursor:pointer;display:flex;align-items:center;gap:2px"><input type="checkbox" class="vgb-cb" value="wafer" '+(VAR_GBY.indexOf('wafer')>=0?'checked':'')+' onchange="toggleGby(\'wafer\')"> Wfr</label>'
    +'<label style="font-size:11px;cursor:pointer;display:flex;align-items:center;gap:2px"><input type="checkbox" class="vgb-cb" value="layout" '+(VAR_GBY.indexOf('layout')>=0?'checked':'')+' onchange="toggleGby(\'layout\')"> Lyt</label>'
    +'<label style="font-size:11px;cursor:pointer;display:flex;align-items:center;gap:2px"><input type="checkbox" class="vgb-cb" value="material" '+(VAR_GBY.indexOf('material')>=0?'checked':'')+' onchange="toggleGby(\'material\')"> Mat</label>';
  var jsPid='\''+pid+'\'';
...
function buildXYTab(){
  ...
  pts.forEach(function(pt){
    ...
    if(cx<ML-5||cx>ML+plotW+5||cy<MT-5||cy>MT+plotH+5)return;
    if(!_dp2[col])_dp2[col]='';
    _dp2[col]+='M'+cx.toFixed(1)+','+cy.toFixed(1)+'m-3.5,0a3.5,3.5,0,1,0,7,0a3.5,3.5,0,1,0,-7,0';
  });
  Object.keys(_dp2).forEach(function(col){p.push('<path d="'+_dp2[col]+'" fill="'+col+'" opacity="0.72"/>');});
...
def generate_html(
    ...
    html = (
        ...
        + '</div>\n'
        + '</div></div>\n'
        # --- RO Distribution tab — 5 collapsible panels ---
        + '<div id="tab-pdly" class="tab-panel" style="overflow-y:auto">'
        + _dist_panels_html
        + '</div>\n'         # close tab-pdly
        + '<div id="tab-xy" class="tab-panel" style="overflow-y:auto">'
        + _xy_panels_html
        + '</div>\n'         # close tab-xy
