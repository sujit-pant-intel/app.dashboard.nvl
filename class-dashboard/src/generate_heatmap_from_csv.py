...
def generate_heatmaps(csv_path, out_dir=None, tbl_path=None, bin_col='INTERFACE_BIN_119325', gui=False, html_only=False, interactive=True, render_wafermap=True, render_ibin_wafermap=True):
    ...
    _HW_FIELD_PREFIXES_HM = ["Sort Partial Wafer ID", "Cell ID", "Unit Tester Id", "Unit Tester Site Id", "Unit TIU", "Thermal Head Id"]
            ...
            if _hw_bin_total > 0:
                try:
                    ...
                    for _, _hr in _hw_grp.iterrows():
                        ...
                        _hw_breakdown_rows.append(_hrow)
                except Exception:
                    pass
...
.lw-row.lw-active td { background:#ddeeff !important; }
                ...
                if _hw_cols_hm and _hw_breakdown_rows:
                    _hw_css = (
                    ...
                    _hw_data_j = _json_hw_hm.dumps({'cols': _hw_cols_hm, 'rows': _hw_breakdown_rows})
                    _hw_js_hm = (
                        '<script>(function(){var d=' + _hw_data_j + ';var cols=d.cols;var rows=d.rows;'
                        'var t=document.getElementById("hm-hw-table");'
                        'var th=cols.map(function(c){return"<th>"+c+"</th>";}).join("")'
                        '+"<th>Count</th><th>% of bin units</th>";'
                        'var tb=rows.map(function(r){return"<tr>"'
                        '+cols.map(function(c){return"<td>"+String(r[c]||"")+"</td>";}).join("")'
                        '+"<td style=\'text-align:right\'>"+r.count+"</td>'
                        '<td style=\'text-align:right\'>"+r.pct+"</td></tr>";}).join("");'
                        't.innerHTML="<table class=\'hm-hw-tbl\'><thead><tr>"+th+"</tr></thead>'
                        '<tbody>"+tb+"</tbody></table>";})()</script>'
                    )
...
{_hw_js_hm}</body></html>"""
    ...
    if render_ibin_wafermap:
        try:
            ...
            generate_all_ibin_wafer_map(csv_path, out_dir=out_dir, bin_col=_wm_col, gui=gui, interactive=interactive)
...
def generate_all_ibin_wafer_map(csv_path, out_dir=None, bin_col=None, gui=False, interactive=True, bindef_path=None):
    """
    ...
    try:
        ...
        _HW_FIELD_PREFIXES_WM = ["Sort Partial Wafer ID", "Cell ID", "Unit Tester Id", "Unit Tester Site Id", "Unit TIU", "Thermal Head Id"]
        _hw_cols_wm = [
            next((c for c in df.columns if c.lower().startswith(pfx.lower())), None)
        ...
        def _draw_wafer_on_ax(ax, wdf, title_str, show_reticle=True, fontscale=1.0, reticle_fontscale=None):
            ...
            for _di in range(len(wx)):
                ...
                # gid encodes label + FB + HW combo index for SVG post-processing
                _rect.set_gid(f'die_{_di}_{wlbls[_di].replace(" ","_")}__FB{wfbs[_di]}__HW{whwidx[_di]}')
                ax.add_patch(_rect)
...
.comp-corner-resize:hover{opacity:1;background:linear-gradient(135deg,transparent 50%,#2980b9 50%)}"""
        ...
        for lot_label in lots:
            ...
            for wafer in wafers:
                ...
                leg = _draw_wafer_on_ax(ax_svg, wdf, title_str, fontscale=0.75, reticle_fontscale=0.75 * 3.025)
            ...
            per_wafer_html = ''
            for _wlabel, _wsvg, _svg_id, _leg_items, _wlcounts in per_wafer_imgs:
                ...
                per_wafer_html += (
                    ...
                    f'<div id="wsvg-wrap-{_wlabel}" style="width:100%">{_wsvg}</div>'
  ...
  <div class="fb-body">
    <div id="ibin-fb-cblist" class="fb-cblist"></div>
...
function ibinFbApply(){
  ...
  toApply.forEach(function(sec){
    sec.querySelectorAll('g[data-bin]').forEach(function(g){
      var db=g.getAttribute('data-bin'),fb=g.getAttribute('data-fb')||'0';
      if(db===_ibinFbBin){
        if(sel.size===0||sel.has(fb)){g.style.opacity='1';g.style.filter='drop-shadow(0 0 3px #ff0)';}
        else{g.style.opacity='0.08';g.style.filter='';}
      }else{g.style.opacity='0.10';g.style.filter='';}
    });
  });
  if(_ibinFbSvgId){var _orig=document.getElementById(_ibinFbSvgId);if(_orig&&!_orig.closest('.wm-wafer-section')){_orig.querySelectorAll('g[data-bin]').forEach(function(g){var db=g.getAttribute('data-bin'),fb=g.getAttribute('data-fb')||'0';if(db===_ibinFbBin){if(sel.size===0||sel.has(fb)){g.style.opacity='1';g.style.filter='drop-shadow(0 0 3px #ff0)';}else{g.style.opacity='0.08';g.style.filter='';}}else{g.style.opacity='0.10';g.style.filter='';}});}}
  ibinFbRenderChart();
...
.hw-btn:hover{background:#d6eaf8}</style>"""
            ...
            if _hw_cols_wm:
                _hw_show_fn = (
                    ...
                    '  var key=activeCols.map(function(c){return String(combo[c]||"");}).join("\\x00");'
                    ...
                    '  +\'</div>\';'
                    'var th=\'<tr><th style="width:30px"></th>\'+["Count","%"].concat(activeCols).map(function(c){return\'<th style="text-align:left;white-space:normal;word-wrap:break-word">\'+c+\'</th>\';}).join("")+\'</tr>\';'
                    'var filterRow=\'<tr><td></td>\'+["Count","%"].concat(activeCols).map(function(c){'
                    ...
                    '    +activeCols.map(function(c){return"<td>"+String(e.combo[c]||"").replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;")+"</td>";}).join("")'
                    '    +\'</tr>\';'
                    '}).join("");'
                    ...
                    'if(_ibinHwGroupByCols.size===0){cb.checked=true;_ibinHwGroupByCols.add(c);}'
                    ...
                    'if(orderedCols.length)_ibinHwGroupByCols.add(orderedCols[0]);'
                    '_ibinHwColFilter={};_ibinHwSel.clear();ibinHwRenderList();}'
...
<button onclick="wpaOpen()" style="padding:5px 14px;font-size:13px;cursor:pointer;border:1px solid #145a32;background:#eafaf1;border-radius:4px;color:#145a32;font-family:Arial,sans-serif;font-weight:bold">&#127759; Wafer Pattern Analysis</button>
...
def main():
    ...
    if wafermap_only:
        generate_all_ibin_wafer_map(csvp, out_dir=outd, gui=gui, interactive=interactive, bindef_path=bindef)
    else:
