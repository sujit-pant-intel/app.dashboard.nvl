...
def add_material_type(
    ...
    log_cb=None,
    hint_lot_col: str | None = None,
    hint_wafer_col: str | None = None,
) -> str:
    ...
    *output_dir* (creating it if needed).  If *output_dir* is None a temp
    ...
    # Prefer caller-supplied hints (from product config JSON)
    if hint_lot_col and hint_lot_col in all_cols and hint_wafer_col and hint_wafer_col in all_cols:
        lot_col, wafer_col = hint_lot_col, hint_wafer_col
    else:
    ...
    if lookup_frames:
        ...
        df_merged = df.merge(
            combined_lookup[['INTEL_LOT7', '_WaferID_num'] + merge_cols_available],
            left_on=['LOT7', 'WAFER2'],
