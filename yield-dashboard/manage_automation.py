"""
manage_automation.py  —  GUI to manage yield-dashboard automation.

Tabs:
  1. Email & Filter   — recipients + per-TP excluded keys (email_config.json)
  2. Run History      — NVL_0H61_YYYYMMDD/ run folders; view/delete old runs
  3. Data Files       — data/programs/*.csv.gz and raw AQUA pull snapshots
  4. Schedule         — Windows Task Scheduler: create, check, run now, remove

Usage:
    python manage_automation.py
    python manage_automation.py --base-dir "\\\\server\\share\\auto"
"""
...
def _discover_keys(base_dir: Path) -> list[str]:
    ...
    # Fallback: Dashboard_*.html at root (older runs / manual copies)
    for p in sorted(base_dir.glob("Dashboard_*.html")):
        stem = p.stem  # e.g. "Dashboard_NCXSDJXL0H61C002618_132322"
        if stem.startswith("Dashboard_"):
            keys.add(stem[len("Dashboard_"):])
...
class AutomationManager(tk.Frame):
    ...
    def _build_email_tab(self) -> None:
        ...
        self.alert_email_var  = tk.StringVar(value=self.cfg.get("email_to_alert",
    ...
    def _build_history_tab(self) -> None:
        ...
        self.hist_tree.heading("tag",    text="Tag",          anchor="w",      command=lambda c="tag":    self._sort_tree(self.hist_tree, c))
        self.hist_tree.heading("folder", text="Run Folder",   anchor="w",      command=lambda c="folder": self._sort_tree(self.hist_tree, c))
        self.hist_tree.heading("date",   text="Date / Time",  anchor="w",      command=lambda c="date":   self._sort_tree(self.hist_tree, c))
        self.hist_tree.heading("tps",    text="TPs",          anchor="center", command=lambda c="tps":    self._sort_tree(self.hist_tree, c))
        ...
        self._hist_ctx = tk.Menu(self, tearoff=0, bg=BG2, fg=FG,
                                 ...
                                 font=FONT_UI, bd=0)
        self._hist_ctx.add_command(label="🌐  Open report.html in Browser",
    ...
    def _refresh_history(self) -> None:
        ...
        # Run folders live under base_dir/output/NVL_0H61_{ts}/
    ...
    def _hist_show_ctx(self, event) -> None:
        """Right-click: select the row under cursor then show context menu."""
    ...
    def _hist_open_html(self, copy_only: bool = False) -> None:
        """Open report.html in the default browser, or copy its file:// URL."""
        import webbrowser
        sel = self.hist_tree.selection()
        if not sel:
            messagebox.showinfo("No run selected",
                                "Select a run folder first.")
            return
        if len(sel) > 1 and not copy_only:
            # Open all selected reports
            opened = 0
            for iid in sel:
                html = Path(iid) / "report.html"
                if html.exists():
                    webbrowser.open(html.as_uri())
                    opened += 1
            self.hist_status.set(
                f"Opened {opened} report(s) in browser." if opened
                else "No report.html found in selected runs."
            )
            return
        ...
        if not html.exists():
            messagebox.showerror("No report",
                                 f"report.html not found in:\n{run_dir}")
            return

        url = html.as_uri()          # file:///C:/... or file:////server/...
        if copy_only:
            ...
            webbrowser.open(url)
            self.hist_status.set(f"Opened in browser: {html.name}  ({run_dir.name})")
    ...
    def _hist_tag(self) -> None:
        ...
        if not sel:
            messagebox.showinfo("No run selected",
                                "Select one or more runs to tag.")
            return
    ...
    def _hist_send_email(self) -> None:
        """Scan all output dirs, collect the latest run per program letter,
        build a combined report from all of them, then send one email."""
        ...
        if not messagebox.askyesno(
            ...
            f"Scan all output dirs and send combined report?\n\n"
        ...
        def _send():
            try:
                ...
                run_pattern = _re.compile(r'^NVL_0H(\d+)([A-Za-z])_(\d{8}_\d{6})$')
                found_prog_keys = set()
                latest_ts = ""
                for d in out_dir.iterdir():
                    if not d.is_dir():
                        continue
                    m2 = run_pattern.match(d.name)
                    if m2:
                        found_prog_keys.add(f"{m2.group(1)}{m2.group(2).upper()}")
                        if m2.group(3) > latest_ts:
                            latest_ts = m2.group(3)

                if not found_prog_keys:
                    self.after(0, lambda: messagebox.showerror(
                        "No data", "No NVL_0H*_* run folders found in output/"))
                    self.after(0, lambda: self.hist_status.set(""))
                    return

                run_ts      = _dt.now().strftime("%Y-%m-%d %H:%M:%S")
                # Sort prog keys by (gen_num, letter) so 62A > 61E > 61A
                letters_str = "+".join(sorted(
                    found_prog_keys,
                    key=lambda k: (int(k[:-1]), k[-1]),
                ))
                tmp = Path(tempfile.mkdtemp(prefix="nvl_resend_"))
                try:
                    # ── Build rich sidebar+history HTML ───────────────────────
    ...
    def _hist_save_report(self) -> None:
        """Build combined report HTML and save to reports/ folder — no email."""
        out_dir = self.base_dir / "output"
        if not out_dir.exists():
            messagebox.showinfo("No output folder",
                                f"No output/ directory found under:\n{self.base_dir}")
    ...
    def _hist_cleanup(self) -> None:
        ...
        per program letter.  Tagged runs are always preserved."""
        ...
        default_n = int(self.cfg.get("keep_runs", 5))

        dlg = tk.Toplevel(self)
        dlg.title("Cleanup Old Runs")
        dlg.configure(bg=BG)
        dlg.resizable(False, False)
        dlg.grab_set()
        dlg.transient(self)
        ...
        tk.Label(dlg, text="Cleanup Old Run Folders", font=FONT_TITLE,
                 bg=BG, fg=ACCENT).pack(padx=16, pady=(14, 2))
        tk.Label(dlg,
                 text="Keeps the most-recent N runs per program letter.\n"
                      "Tagged runs (🏷) are always preserved.",
                 font=("Segoe UI", 9), bg=BG, fg=FG_DIM, justify="center").pack(padx=16)

        # ── Keep N spinbox ────────────────────────────────────────────────────
        row_n = tk.Frame(dlg, bg=BG)
        row_n.pack(padx=16, pady=(10, 4))
        tk.Label(row_n, text="Keep last", font=FONT_UI, bg=BG, fg=FG).pack(side="left")
        keep_var = tk.IntVar(value=default_n)
        spin = tk.Spinbox(row_n, from_=1, to=50, textvariable=keep_var, width=5,
                          font=FONT_UI, bg=BG2, fg=FG, buttonbackground=BG3,
                          insertbackground=FG, relief="flat")
        spin.pack(side="left", padx=6)
        tk.Label(row_n, text="runs per letter", font=FONT_UI, bg=BG, fg=FG).pack(side="left")

        # ── Preview text area ─────────────────────────────────────────────────
        frm = tk.LabelFrame(dlg, text="  Folders that will be deleted  ",
        ...
        tk.Label(dlg, textvariable=status_var, font=("Segoe UI", 9),
                 bg=BG, fg=FG_DIM).pack(padx=16, pady=(0, 2))

        def _do_preview(*_) -> list[Path]:
            try:
                n = keep_var.get()
            except Exception:
                return []
            ...
            for letter in sorted(letter_groups):
                folders = sorted(letter_groups[letter],
            ...
            if candidates:
                ...
                status_var.set(
                    f"{len(candidates)} folder(s) to delete — "
                    f"~{total_sz / 1_048_576:.1f} MB freed"
                )
            else:
                txt.insert("end", "  (nothing to delete with these settings)\n")
                status_var.set("Nothing to delete.")
            txt.configure(state="disabled")
            return candidates

        # Run preview whenever N changes
        keep_var.trace_add("write", _do_preview)
        candidates_ref: list[Path] = _do_preview()

        def _on_delete() -> None:
            candidates = _do_preview()   # re-compute in case N changed
            if not candidates:
                messagebox.showinfo("Nothing to delete",
                                    "No old runs to delete with current settings.",
                                    parent=dlg)
                return

            tag_note = " (tagged runs are preserved)"
            if not messagebox.askyesno(
                "Confirm Cleanup",
                f"Permanently delete {len(candidates)} run folder(s){tag_note}?\n\n"
                + "\n".join(f"  {d.name}" for d in candidates[:10])
            ...
            # Save keep_runs setting back to config
            try:
                self.cfg["keep_runs"] = keep_var.get()
            ...
            self.hist_status.set(
                f"Cleanup: deleted {deleted} run folder(s)." +
        ...
        self._btn(btn_row, "🔍 Preview",  _do_preview,
                  bg="#263950", fg=ACCENT).pack(side="left", padx=(0, 6))
        self._btn(btn_row, "🗑 Delete Old Runs", _on_delete,
    ...
    def _build_data_tab(self) -> None:
        ...
        self.raw_tree.heading("filename", text="File",     anchor="w", command=lambda c="filename": self._sort_tree(self.raw_tree, c))
        self.raw_tree.heading("size",     text="Size",     anchor="e", command=lambda c="size":     self._sort_tree(self.raw_tree, c))
        self.raw_tree.heading("modified", text="Modified", anchor="w", command=lambda c="modified": self._sort_tree(self.raw_tree, c))
        ...
        frm_prog = tk.LabelFrame(p, text="  Per-Program Data Cache  (data/programs/)  ",
                                 font=FONT_UI, bg=BG, fg=ACCENT, bd=1, relief="groove")
        frm_prog.pack(fill="both", expand=True, padx=12, pady=(0, 4))
        self.prog_tree = self._make_tree(frm_prog, ("filename", "variant", "size", "modified"))
        self.prog_tree.heading("filename", text="File",     anchor="w", command=lambda c="filename": self._sort_tree(self.prog_tree, c))
        self.prog_tree.heading("variant",  text="Variant",  anchor="w", command=lambda c="variant":  self._sort_tree(self.prog_tree, c))
        self.prog_tree.heading("size",     text="Size",     anchor="e", command=lambda c="size":     self._sort_tree(self.prog_tree, c))
        self.prog_tree.heading("modified", text="Modified", anchor="w", command=lambda c="modified": self._sort_tree(self.prog_tree, c))
        self.prog_tree.column("filename", width=340, stretch=True)
        self.prog_tree.column("variant",  width=70,  stretch=False)
    ...
    def _sched_rerun(self) -> None:
        """Open a Rerun dialog: optional --keys + --local-csv filter, then run
