"""yield_pipeline.py — Merged yield pipeline (mixin orchestration + CLI).

Consolidates: _pipeline_constants, _pipeline_server, _pipeline_html,
  _pipeline_ui, _pipeline_runner, _loader, pipeline (PipelineFrame),
  yield_pipeline (CLI orchestrator)

Public API (unchanged):
  PipelineFrame   — Tkinter GUI frame (from pipeline.py)
  OpenerServer    — HTTP file-opener (from _pipeline_server.py)
  main()          — CLI entry point (from yield_pipeline.py)
"""
from __future__ import annotations

# ════════════════════════════════════════════════════════════════
# (formerly _pipeline_constants.py)
# ════════════════════════════════════════════════════════════════
import os
import sys
...
_LOADER = os.path.join(_SRC_DIR, 'yield_pipeline.py')

SICC_UPM_SCRIPT      = os.path.normpath(os.path.join(_ROOT_DIR, '..', 'sicc_upm', 'sicc_upm.py'))
SICC_CDYN_UPM_SCRIPT = os.path.normpath(os.path.join(_ROOT_DIR, '..', 'sicc_cdyn_upm', 'sicc_cdyn_upm.py'))
    ...
    def _build_pareto_html(self, resolved_csv, bindef_csv, out_dir, tag=None, dashboard_html=None, bucket_json=None):
        ...
        try:
            ...
            try:
                ...
                _log_dd0.getLogger('yield_data').setLevel(_log_dd0.WARNING)
            ...
            if _wm_files and _wm_combined_path.exists() and not wpa_only:
                ...
                try:
                    ...
                    _wm_combined_html = (
                        ...
                        '.wm-pat-tile-ph{min-width:0}\n'
                        '.wm-wlbl{font-size:11px;font-weight:bold;color:#2c3e50;text-align:center;margin-bottom:3px}\n'
                        '.wm-wlbl-ico{display:inline-block;flex-shrink:0;line-height:1;vertical-align:middle}\n'
                        ...
                        +'        <button onclick="_wmPatSetZoom(-1)" style="background:rgba(255,255,255,.15);border:1px solid rgba(255,255,255,.4);color:#fff;font-size:13px;cursor:pointer;padding:0 7px;border-radius:4px;margin-left:6px;line-height:1.6" title="Zoom out">&#8722;</button>\n'
                        +'        <span id="wm-zoom-pct" style="font-size:11px;color:rgba(255,255,255,0.85);min-width:32px;display:inline-block;text-align:center">100%</span>\n'
                        +'        <button onclick="_wmPatSetZoom(+1)" style="background:rgba(255,255,255,.15);border:1px solid rgba(255,255,255,.4);color:#fff;font-size:13px;cursor:pointer;padding:0 7px;border-radius:4px;line-height:1.6" title="Zoom in">&#43;</button>\n'
                        +'        <button onclick="_wmPatResetZoom()" style="background:rgba(255,255,255,.15);border:1px solid rgba(255,255,255,.4);color:#fff;font-size:11px;cursor:pointer;padding:0 7px;border-radius:4px;margin-left:4px;line-height:1.6" title="Reset zoom">&#8635;</button>\n'
                        +'      </div>\n'
                        ...
                        '            <div class="wm-pat-maps-wrap"><div id="wm-pat-maps-scale" style="transform-origin:top left"><div class="wm-pat-maps" id="wm-pat-maps"></div></div>\n'
                        '            </div>\n'
                        '          </div>\n'
...
'}\n'
                        ...
                        '/* CSS-transform zoom — same mechanism as the IBIN wafer map: scale the whole map area (text included) and resize its box so the scroll container reflows. */\n'
                        'function _wmPatApplyZoom(){\n'
                        '  var wrap=document.querySelector(".wm-pat-maps-wrap");\n'
                        '  var sc=document.getElementById("wm-pat-maps-scale");\n'
                        '  if(!wrap||!sc)return;\n'
                        '  var z=_wmPatZoom;\n'
                        '  var avail=Math.max(220,wrap.clientWidth-18);\n'
                        '  sc.style.transformOrigin="top left";\n'
                        '  sc.style.transform="scale("+z+")";\n'
                        '  sc.style.width=Math.round(avail/z)+"px";\n'
                        '  sc.style.marginBottom=Math.round(sc.offsetHeight*(z-1))+"px";\n'
                        '}\n'
                        'function _wmPatRefreshTiles(){\n'
                        ...
                        '  });\n'
                        '}\n'
                        'function _wmPatSetZoom(dir){_wmPatZoom=Math.min(4,Math.max(0.25,Math.round((dir>0?_wmPatZoom*1.25:_wmPatZoom/1.25)*100)/100));_wmPatSyncZoom();}\n'
                        'function _wmPatResetZoom(){_wmPatZoom=1;_wmPatSyncZoom();}\n'
                        'function _wmPatSyncZoom(){\n'
                        '  var z=document.getElementById("wm-zoom-pct");if(z)z.textContent=Math.round(_wmPatZoom*100)+"%";\n'
                        '  _wmPatApplyZoom();\n'
                        '  _wmPatRefreshTiles();\n'
                        '  if(_wmPatBuildBinRow.lastMapKeys)_wmBuildModeMap(_wmPatBuildBinRow.lastMapKeys);\n'
                        '}\n'
                        'window.addEventListener("resize",function(){_wmPatApplyZoom();});\n'
                        ...
                        '    var _cvr=Math.max(1,_wmPatZoom);\n'
                        '    cv.width=Math.round(W*_cvr);cv.height=Math.round(H*_cvr);\n'
                        '    cv.style.width=W+"px";cv.style.height=H+"px";cv.style.display="block";\n'
                        '    var ctx=cv.getContext("2d");ctx.scale(_cvr,_cvr);\n'
                        '    dies.forEach(function(d){\n'
                        ...
                        '      var _cvFill=_wmIbColor(ibKey),_cvAlpha=binOn?1:0.08;\n'
                        '      if(_wmUpmOverlay){\n'
                        '        var _ue2=WM_PAT.upmCols&&WM_PAT.upmCols[0];var _ut2=Array.isArray(_ue2)?_ue2[1]:null;\n'
                        '        var _ur2=d[4]!==undefined&&d[4]!==null?d[4]:null;\n'
                        '        if(_wmIsFail(ibKey)){_cvFill=(_ur2!==null&&_ut2)?_wmUpmColor(_ur2/_ut2*100):"#c8c8c8";_cvAlpha=1;}\n'
                        '        else if(_ur2!==null&&_ut2){_cvFill=_wmUpmColor(_ur2/_ut2*100);_cvAlpha=1;}\n'
                        '        else{_cvFill="#c8c8c8";_cvAlpha=0.5;}\n'
                        '      }\n'
                        '      ctx.globalAlpha=_cvAlpha;\n'
                        '      ctx.fillStyle=_cvFill;\n'
...
'function _wmPatBuildCtrl(allKeys){\n'
                        ...
                        '        var _icon=_cb.length?\'<span data-pk="'+pk+'" style="color:#c0392b;font-size:9px;vertical-align:middle;cursor:pointer" title="Click to see yield misses" onclick="event.stopPropagation();_wmShowIconTip(this)">&#9888;</span>\':\'\';\n'
                        '        return \'<div class="wm-wlbl" style="cursor:pointer" onclick="_wmShowCriteriaTable(\\\''+pk+'\\\',event)">\'+mLot+" W"+mWfr+mProgLbl+_icon+\'</div>\'\n'
                        '          +(mMat?\'<div style="font-size:9px;color:#8e6a2a;margin-bottom:2px">\'+mMat+\'</div>\':\'\')\n'
                        '      })()\n'
                        ...
                        '  }\n'
                        '  var _mz=_wmPatZoom;\n'
                        '  var svgStr=`<div style="width:${Math.round(W*_mz)}px;height:${Math.round(H*_mz)}px">`\n'
                        '    +`<svg id="wmm-svg" width="${W}" height="${H}" xmlns="http://www.w3.org/2000/svg" style="display:block;transform:scale(${_mz});transform-origin:top left">`\n'
...
class PipelineUIMixin:
    def _build_ui(self):
        ...
        # ── Parametric / PCM Options ─── hidden until sort-parametric rearchitected
        pcm_frm = _lf(_right, 'Parametric Dashboard', '#3498db')
        pcm_frm.pack_forget()  # hide entire section from GUI
        ...
        self.run_parametric_var = tk.BooleanVar(value=False)  # disabled — rearchitecting
        # Parametric checkbox hidden until sort-parametric is rearchitected
        tk.Checkbutton(
    ...
    def reset_fields(self):
        ...
        try: self.run_parametric_var.set(False)  # disabled — rearchitecting
        except Exception: pass
        # Data CSV listbox
        try:
            self._data_csv_lb.delete(0, tk.END)
...
class PipelineRunnerMixin:
    def run_pipeline(self):
        ...
        _pipeline_script = os.path.join(_SRC_DIR, 'yield_pipeline.py')
        ...
        def _run_in_thread():
            try:
                ...
                proc = subprocess.Popen(
                    [sys.executable, '-B', _pipeline_script, '--json', _tmp_json_path[0]],
    ...
    def _run_sicc_py_headless(self, csv_path: str, out_dir: str):
        ...
        try:
            ...
            _sicc_py_src = _PP_py(_SRC_DIR).parent / 'sicc_cdyn_upm'
            if _sicc_py_src.is_dir() and str(_sicc_py_src) not in _sys_py.path:
                _sys_py.path.insert(0, str(_sicc_py_src))
            from sicc_cdyn_upm import load_config as _lc_py, process_csv as _pc_py

            _cfg_path = _PP_py(_SRC_DIR).parent / 'collateral' / 'sicc_cdyn_testlist.json'
...
_run_root = _here.parent  # up from <module>/ to run root
for _src_dir in _run_root.glob('*'):
    if _src_dir.is_dir() and not _src_dir.name.startswith('.') and str(_src_dir) not in sys.path:
        sys.path.insert(0, str(_src_dir))


# ════════════════════════════════════════════════════════════════
# (formerly pipeline.py — PipelineFrame entry point)
# ════════════════════════════════════════════════════════════════
...
if __name__ == '__main__':
    ...
    if _known.json_file:
        ...
        _cfg['run_parametric'] = False  # disabled — rearchitecting sort-parametric
        ...
        _loader = _src_dir / 'yield_analysis.py'
        ...
        _cmd = [sys.executable, str(_src_dir / 'yield_pipeline.py'),
            ...
            try:
                ...
                if _sort_lot_col and _sort_wafer_col:
                    ...
                    for _mcp in _all_mat_csv:
                        ...
                        # Accept 'INTEL_LOT' (without 7-char suffix) as alias for 'INTEL_LOT7'
                        if 'INTEL_LOT7' not in _df_m.columns and 'INTEL_LOT' in _df_m.columns:
                            _df_m = _df_m.rename(columns={'INTEL_LOT': 'INTEL_LOT7'})
                        if ('INTEL_LOT7' in _df_m.columns and 'WaferID' in _df_m.columns
                            ...
                            # WaferID may be numeric ("3") or formatted ("Q615S1B-03") — extract trailing digits
                            _wid_raw = _df_m['WaferID'].astype(str).str.strip()
                            _wid_num = _mpd_mat.to_numeric(_wid_raw, errors='coerce')
                            _wid_suffix = _mpd_mat.to_numeric(
                                _wid_raw.str.extract(r'(\d+)$', expand=False), errors='coerce')
                            _df_m['_m_wafer'] = _wid_num.where(_wid_num.notna(), _wid_suffix)
                            _mat_frames.append(
        ...
        _sicc_script = (
            ...
            'from sicc_cdyn_upm import load_config as _lc, process_csv as _pc\n'
            'from sicc_cdyn_upm import generate_html_svg as _gh\n'
            ...
            '        for _key in ("siccList","siccTotalList","cdynList","upmInfo","SiccTableConfig","cdynTableConfig","upmTableConfig"):\n'
...
# ════════════════════════════════════════════════════════════════
# (formerly yield_pipeline.py — CLI orchestrator)
# ════════════════════════════════════════════════════════════════
import json
import argparse
import subprocess
...
BASE: Path | None = None
_LOADER = str(ROOT / 'yield_pipeline.py')  # _loader.py merged into yield_pipeline
...
def run_parse_bindef(bindef_path: Path, out_csv: Path) -> int:
    if getattr(sys, 'frozen', False):
        ...
        try:
            ...
            return 1
    # call in-process via yield_data (parse_bindef_to_crystalball merged there)
    import yield_data as _yd
    out_arg = str(out_csv)
    try:
        rc = _yd._parse_bindef_main(['--bindef', str(bindef_path), '--out', out_arg])
        return rc or 0
    except SystemExit as e:
        return e.code or 0
    except Exception as e:
...
def run_get_dd(data_csv: Path, bindef_csv: Path, dashboard: Path, out_dir: Path | None = None) -> int:
    if getattr(sys, 'frozen', False):
        ...
        try:
            ...
            sys.argv = saved_argv
    # call in-process via yield_data (get_dd_update merged there)
    import sys as _sys, yield_data as _yd
    _saved_argv = _sys.argv
    _argv = ['get_dd_update.py', '--data', str(data_csv), '--bin_defs', str(bindef_csv), '--dashboard', str(dashboard)]
    if out_dir:
        _argv += ['--outdir', str(out_dir)]
    _sys.argv = _argv
    try:
        _yd._get_dd_main()
        return 0
