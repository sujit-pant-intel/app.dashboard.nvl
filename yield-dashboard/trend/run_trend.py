...
_TREND_SCRIPT = _REPO_ROOT / "yield-dashboard" / "yld" / "trend_chart.py"
...
# Maps devrevstep prefix → (output filename stem, per-product folder name)
_DEVREVSTEP_SPLITS = {
    "8PF6CV": "NVL816-Yield-Trend-Report",
    "8PF5CV": "NVL816-BLLC-Yield-Trend-Report",
    "8PL7CV": "NVLG-512-Yield-Trend-Report",
}

# Maps devrevstep prefix → per-product reports subfolder
_PREFIX_TO_PRODUCT = {
    "8PF6CV": "NVL816",
    "8PF5CV": "NVL816-BLLC",
    "8PL7CV": "NVLG-512",
...
def _build_email_body(run_ts: str, index_unc: str, index_uri: str) -> str:
    """Build simple HTML email body with a single link to the reports index."""
    return f"""<!DOCTYPE html><html><body style="font-family:Segoe UI,Arial;background:#1e1e1e;color:#d4d4d4;max-width:600px">
<h2 style="color:#4fc3f7">NVL Trend Report</h2>
<p style="color:#9e9e9e">{run_ts}</p>
...
def main() -> None:
    ...
    try:
        ...
        for _pfx, _csv_file in split_map.items():
            _prod_folder     = trend_dir     / _PREFIX_TO_PRODUCT.get(_pfx, _pfx)
            _unc_prod_folder = unc_trend_dir / _PREFIX_TO_PRODUCT.get(_pfx, _pfx)
            _prod_folder.mkdir(parents=True, exist_ok=True)
            _out_html = _prod_folder     / (_csv_file.stem + ".html")  # I/O path
            _unc_html = _unc_prod_folder / (_csv_file.stem + ".html")  # UNC path for links
        ...
        subject = "NVL Trend Report — " + ts_label
        reports_unc = str(base_dir / "reports").replace("/", "\\")
        reports_uri = (base_dir / "reports").as_uri()
        body = _build_email_body(run_ts, reports_unc, reports_uri)
