...
Also includes the reports/ index-page builder (merged from former generate_index.py).
...
# ─────────────────────────────────────────────────────────────────────────────
# Reports index  (merged from former automation/generate_index.py)
# ─────────────────────────────────────────────────────────────────────────────
...
def build_report_index(base_dir: Path) -> Path:
    """Scan reports/ and write a static index.html of Scan_Report_*.html files."""
    reports_dir = base_dir / "reports"
    reports_dir.mkdir(parents=True, exist_ok=True)
    _reports_unc = str(reports_dir)
    ...
    for i, f in enumerate(files):
        ...
        try:
            ...
            sz    = "–"
            mtime = "–"
        unc   = "file:////" + _reports_unc.replace("\\", "/").lstrip("/") + "/" + f.name
...
<body>
  ...
  <div class="card">
    ...
    <code style="color:var(--acc);font-size:11px">{_reports_unc}</code>
  </div>
...
def main() -> None:
    ...
    if not args.dry_run and all_results:
        ...
        _idx = build_report_index(base_dir)
        _log(f"Reports index updated: {_idx}")
